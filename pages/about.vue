<template>
    <div class="flex justify-center flex-col">
        <h2 class="scroll-m-20 pb-2 text-3xl font-bold tracking-tight transition-colors first:mt-0">
            What is Watchtower?
        </h2>
        <p class="leading-7">
            By leveraging tools like Google's Page Speed Insights API, we monitor and analyze key metrics across
            numerous government portals. Our goal is to provide reports that highlight improvements and identify areas
            for enhancement. Through vigilant oversight, Watchtower promotes better digital governance and fosters
            public trust in governmental online services.
        </p>
        <div class="max-w-4xl mx-auto mt-6">
            <Button size="lg" class="rounded-full" @click="sendEmail">Get in Touch</Button>
        </div>
        <div class="px-96 py-10">
            <div class="border-b ">

            </div>
        </div>
        <div>
            <h2 class="scroll-m-20 pb-2 text-3xl font-bold tracking-tight transition-colors first:mt-0">
                Our Team
            </h2>
            <div class="grid grid-cols-1 md:grid-cols-3 sm:grid-cols-12 gap-10 justify-items-center mt-6 md:px-64 py-10">
                <div class="flex flex-col items-center">
                    <img src="@/assets/images/team/boss.jpeg" alt="Avatar 1"
                        class="rounded-full w-24 h-24 object-cover">
                    <p class="mt-2 text-lg font-semibold">Ali Raza</p>
                    <a href="https://aliirz.com/" target="_blank" class="flex inline text-gray-600 pt-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" viewBox="0 0 1200 1200">
                            <path fill="#e11d47"
                                d="M600 0C268.629 0 0 268.629 0 600s268.629 600 600 600s600-268.629 600-600S931.371 0 600 0M281.154 301.634h637.653v596.732H281.154zm78.611 77.963V820.44h480.471V379.597zm52.393 56.231H786.55v195.479H412.158zm0 229.369h106.041v106.041H412.158zm134.156 0h106.078v106.041H546.314zm134.157 0H786.55v106.041H680.471z" />
                        </svg>
                        &nbsp;aliirz
                    </a>
                </div>
                <div class="flex flex-col items-center">
                    <img src="@/assets/images/team/talha.jpeg" alt="Avatar 2"
                        class="rounded-full w-24 h-24 object-cover">
                    <p class="mt-2 text-lg font-semibold">Talha Ashraf</p>
                    <a href="https://github.com/Talha-Ashraf420" target="_blank" class="flex inline text-gray-600 pt-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" viewBox="0 0 32 32">
                            <path fill="#e11d47"
                                d="M16 .396c-8.839 0-16 7.167-16 16c0 7.073 4.584 13.068 10.937 15.183c.803.151 1.093-.344 1.093-.772c0-.38-.009-1.385-.015-2.719c-4.453.964-5.391-2.151-5.391-2.151c-.729-1.844-1.781-2.339-1.781-2.339c-1.448-.989.115-.968.115-.968c1.604.109 2.448 1.645 2.448 1.645c1.427 2.448 3.744 1.74 4.661 1.328c.14-1.031.557-1.74 1.011-2.135c-3.552-.401-7.287-1.776-7.287-7.907c0-1.751.62-3.177 1.645-4.297c-.177-.401-.719-2.031.141-4.235c0 0 1.339-.427 4.4 1.641a15.4 15.4 0 0 1 4-.541c1.36.009 2.719.187 4 .541c3.043-2.068 4.381-1.641 4.381-1.641c.859 2.204.317 3.833.161 4.235c1.015 1.12 1.635 2.547 1.635 4.297c0 6.145-3.74 7.5-7.296 7.891c.556.479 1.077 1.464 1.077 2.959c0 2.14-.02 3.864-.02 4.385c0 .416.28.916 1.104.755c6.4-2.093 10.979-8.093 10.979-15.156c0-8.833-7.161-16-16-16z" />
                        </svg>
                        &nbsp;talha-ashraf420
                    </a>
                </div>
                <div class="flex flex-col items-center">
                    <img src="@/assets/images/team/rida.jpeg" alt="Avatar 3"
                        class="rounded-full w-24 h-24 object-cover">
                    <p class="mt-2 text-lg font-semibold">Rida Ashraf</p>
                    <a href="https://www.behance.net/ridaashraf1" target="_blank"
                        class="flex inline text-gray-600 pt-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="1.5em" height="1.5em" viewBox="0 0 1024 1024">
                            <path fill="#e11d47"
                                d="M420.3 470.3c8.7-6.3 12.9-16.7 12.9-31c.3-6.8-1.1-13.5-4.1-19.6c-2.7-4.9-6.7-9-11.6-11.9a44.8 44.8 0 0 0-16.6-6c-6.4-1.2-12.9-1.8-19.3-1.7h-70.3v79.7h76.1c13.1.1 24.2-3.1 32.9-9.5m11.8 72c-9.8-7.5-22.9-11.2-39.2-11.2h-81.8v94h80.2c7.5 0 14.4-.7 21.1-2.1a50.5 50.5 0 0 0 17.8-7.2c5.1-3.3 9.2-7.8 12.3-13.6c3-5.8 4.5-13.2 4.5-22.1c0-17.7-5-30.2-14.9-37.8M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448s448-200.6 448-448S759.4 64 512 64m86.5 286.9h138.4v33.7H598.5zM512 628.8a89.5 89.5 0 0 1-27 31c-11.8 8.2-24.9 14.2-38.8 17.7a167.4 167.4 0 0 1-44.6 5.7H236V342.1h161c16.3 0 31.1 1.5 44.6 4.3c13.4 2.8 24.8 7.6 34.4 14.1c9.5 6.5 17 15.2 22.3 26c5.2 10.7 7.9 24.1 7.9 40c0 17.2-3.9 31.4-11.7 42.9c-7.9 11.5-19.3 20.8-34.8 28.1c21.1 6 36.6 16.7 46.8 31.7c10.4 15.2 15.5 33.4 15.5 54.8c0 17.4-3.3 32.3-10 44.8M790.8 576H612.4c0 19.4 6.7 38 16.8 48c10.2 9.9 24.8 14.9 43.9 14.9c13.8 0 25.5-3.5 35.5-10.4c9.9-6.9 15.9-14.2 18.1-21.8h59.8c-9.6 29.7-24.2 50.9-44 63.7c-19.6 12.8-43.6 19.2-71.5 19.2c-19.5 0-37-3.2-52.7-9.3c-15.1-5.9-28.7-14.9-39.9-26.5a121.2 121.2 0 0 1-25.1-41.2c-6.1-16.9-9.1-34.7-8.9-52.6c0-18.5 3.1-35.7 9.1-51.7c11.5-31.1 35.4-56 65.9-68.9c16.3-6.8 33.8-10.2 51.5-10c21 0 39.2 4 55 12.2a111.6 111.6 0 0 1 38.6 32.8c10.1 13.7 17.2 29.3 21.7 46.9c4.3 17.3 5.8 35.5 4.6 54.7m-122-95.6c-10.8 0-19.9 1.9-26.9 5.6s-12.8 8.3-17.2 13.6a48.4 48.4 0 0 0-9.1 17.4c-1.6 5.3-2.7 10.7-3.1 16.2H723c-1.6-17.3-7.6-30.1-15.6-39.1c-8.4-8.9-21.9-13.7-38.6-13.7" />
                        </svg>
                        &nbsp;ridaashraf1
                    </a>
                </div>
            </div>
        </div>
    </div>
</template>

<script lang="ts">
export default {
    methods: {
        sendEmail() {
            window.open('mailto:info@codeforpakistan.org')
        }
    }
}
</script>