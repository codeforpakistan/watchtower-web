<template>
  <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex gap-2">
            <Button
              ...
              @click="severity = null; loadShameWall()"
            >
              All
            </Button>
            <Button
              :variant="severity === 'critical' ? 'destructive' : 'outline'"
              @click="severity = 'critical'; loadShameWall()"
            >
              Critical
            </Button>
            <Button
              :variant="severity === 'high' ? 'destructive' : 'outline'"
              @click="severity = 'high'; loadShameWall()"
            >
              High
            </Button>
            <Button
              :variant="severity === 'medium' ? 'outline' : 'outline'"
              @click="severity = 'medium'; loadShameWall()"
            >
              Medium
            </Button>
          </div>
    ...
    <div v-else class="grid gap-6 md:grid-cols-2">
      <Card
        v-for="site in shameWall?.shame_wall || []"
        :key="site.id"
      ...
      >
          ...
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <CardTitle class="flex items-center gap-2">
                {{ site.name }}
                <span
                  v-if="site.latest_report?.ssl_shame_severity"
                  :class="{
                    'text-red-600': site.latest_report.ssl_shame_severity === 'critical',
                    'text-orange-600': site.latest_report.ssl_shame_severity === 'high',
                    'text-yellow-600': site.latest_report.ssl_shame_severity === 'medium'
                  }"
                  class="text-sm font-normal"
                >
                  ({{ site.latest_report.ssl_shame_severity }})
                </span>
              </CardTitle>
              <CardDescription>
        ...
        <CardContent>
          ...
          <div class="space-y-3">
            ...
            <ul class="space-y-2">
              <li
                v-for="(reason, index) in site.shame_reasons"
                :key="index"
      ...
      <!-- Empty State -->
      <Card v-if="!shameWall?.shame_wall?.length" class="col-span-full text-center py-12">
        <CardContent>
          <div class="text-4xl mb-4">🎉</div>
          <h3 class="text-xl font-bold mb-2">No Shame-Worthy Sites Found!</h3>
    ...
    <!-- Stats Footer -->
    <Card class="mt-8" v-if="!loading && shameWall">
      <CardContent class="p-6">
        <div class="flex justify-between items-center flex-wrap gap-4">
          <div class="text-sm">
            <span class="font-semibold text-red-600">{{ shameWall.total_count }}</span>
            <span class="text-muted-foreground"> websites with critical issues</span>
          </div>
          <div class="text-sm text-muted-foreground">
            {{ shameWall.description }}
          </div>
        </div>
      </CardContent>
    </Card>
...
const { getShameWall } = useWatchtowerApi()

const shameWall = ref<any>(null)
const loading = ref(true)
const severity = ref<'critical' | 'high' | 'medium' | null>(null)

const loadShameWall = async () => {
  loading.value = true
  try {
    const data = await getShameWall({
      severity: severity.value || undefined,
      limit: 50
    })
    shameWall.value = data
  } catch (error) {
    console.error('Failed to load shame wall:', error)
  } finally {
    loading.value = false
  }
}
...
// SEO
useHead({
  title: 'Wall of Shame - Watchtower',
  meta: [
